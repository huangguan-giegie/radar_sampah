...
export default function SharedEventScreen() {
  ...
  const [target, setTarget] = useState<Awaited<ReturnType<typeof getCleanupTargetRecord>>>(null);
  const [cleanups, setCleanups] = useState<Awaited<ReturnType<typeof eventCleanups>>>([]);
  useEffect(() => {
    let active = true;
    Promise.all([getCleanupEvent(eventId), eventCleanups(eventId)]).then(async ([eventResult, cleanupRows]) => {
      if (!active) return;
      setEvent(eventResult);
      setCleanups(cleanupRows);
      setTarget(eventResult ? await getCleanupTargetRecord(eventResult.beachId) : null);
    });
    return () => { active = false; };
  }, [eventId]);
