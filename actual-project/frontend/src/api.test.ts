// Tests for src/api.ts, which has a live-API branch and a mock branch for demos.
// The live branch is checked by faking fetch and inspecting exactly what we
// would have sent, so a broken contract fails here and not during a demo. The
// mock branch is checked for the rules nobody can see by clicking around: the
// score, the local day, the duplicate rule. The first test matters most - it
// proves a photo loses its location before it is uploaded, as the UI promises.
...
describe('真实 API contract', () => {
  ...
  it('does not send auth headers for the public beach map request', async () => {
  ...
  // The species model takes a beach's broad-area coordinate, never the
  // reporter's own position. This pins down the URL and the two field names
  // the backend expects.
