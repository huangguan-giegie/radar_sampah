...
export interface SpeciesLikelihood {
   ...
   * what it printed was our wiring status on a public page.
   */
...
/**
 * The whole answer from the species-distribution endpoint, built from a
 * packaged OBIS snapshot rather than a live query.
 *
 * calibratedProbability is typed as the literal false so a payload that ever
 * claims otherwise fails to compile. insideMalaysianEez says whether the
 * coordinate was inside the area the model was fitted on. All of it is
 * context: none of it touches the litter score.
 */
