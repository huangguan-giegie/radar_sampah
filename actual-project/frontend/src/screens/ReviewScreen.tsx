...
export default function ReviewScreen() {
  ...
  useEffect(() => {
    getBeaches().then(setBeaches).catch(() => setBeaches([]));
  }, []);

  useEffect(() => {
    let active = true;
    getMyReports()
      .then((reports) => { if (active) setDuplicateMatch(findExactDuplicateReport(draft, reports)); })
      .catch(() => { if (active) setDuplicateMatch(null); });
    return () => { active = false; };
  }, [draft]);

  const beach = beaches.find((candidate) => candidate.id === draft.beachId);
  const photoUrl = draft.photo?.previewUrl || photoPreviewUrl(draft.photo?.photoKey) || draft.existingPhotoUrl;
  ...
  async function submit() {
    ...
    try {
      ...
      setError(validationError instanceof Error ? validationError.message : 'This report is missing a required field.');
      return;
    }
    ...
    setError(null);
    try {
      const saved = submission.kind === 'update'
  ...
  const row = (label: string, value: string, action?: () => void, badge?: string) => (
    ...
    >
      <span style={{ width: 92, flex: 'none', fontSize: 11.5, fontWeight: 600, color: C.dim }}>{label}</span>
      ...
      {badge && <StatusBadge status="duplicate" style={{ marginLeft: 'auto' }}>{badge}</StatusBadge>}
    </button>
  );

  const confirmedCategories = Object.keys(draft.quantities) as LitterCategory[];

  return (
    <div className="screen scroll-y" style={{ zIndex: 26 }}>
      <div className="anim-fade-up pt-page measure" style={{ paddingInline: 20, paddingBottom: 'calc(var(--safe-bottom) + 32px)', display: 'flex', flexDirection: 'column', gap: 16 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <BackButton onClick={backToDetails} />
          <StepBadge>STEP 3 OF 3 · REVIEW</StepBadge>
        </div>

        <div><div style={{ fontSize: 29, fontWeight: 640, letterSpacing: '-.7px' }}>Almost there</div></div>

        <div style={{ position: 'relative', height: 150, borderRadius: 24, overflow: 'hidden', background: '#A19C90' }}>
          {photoUrl && <img src={photoUrl} alt="Report evidence" style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />}
        ...
        <div style={{ background: C.white, border: `1px solid ${C.line}`, borderRadius: 24, overflow: 'hidden' }}>
          ...
          {confirmedCategories.length === 0
            ...
            : confirmedCategories.map((category) => row(category, draft.quantities[category] ?? 'Not selected', backToDetails))}
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', background: C.tint, borderRadius: 16, padding: '13px 14px' }}>
          ...
          <div style={{ fontSize: 12, lineHeight: 1.55, color: C.slate }}>
            Duplicate or incomplete reports are excluded from the severity calculation — the same rule for every beach.
          </div>
        </div>
