...
`main` commit `b1e5baf` contains the DiveSafe MY switch, live smoke evidence and
the editable deck link. The live API returned the health, site, species and
briefing responses after the Render redeploy. A synthetic profile, demo
recognition fallback and sighting were created, then read back from PostgreSQL;
the fallback response stayed marked `needs_user_confirmation: true`.
