...
export async function recordCheckInData(eventId: string, participantId: string, state: CheckInState): Promise<CleanupEvent> {
  if (USE_MOCK) return recordCheckIn(eventId, participantId, state);
...
export async function fetchCleanupTarget(beachId: string): Promise<CleanupTarget | null> {
  ...
  return apiRequest<CleanupTarget | null>(`/cleanup-targets/${encodeURIComponent(beachId)}`);
}
