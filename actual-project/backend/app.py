...
# hx：四个海滩的正式数据表（对齐数据库负责人的 schema.sql）；以前海滩信息只从
# 本地 beaches.json 读，现在数据库里也有一份，load_beaches() 会优先读这张表，
# JSON 文件降级成本地测试兜底用
beaches_table = Table(
    "beaches",
    metadata,
...
# hx：IUCN 威胁类别的字典表（比如"栖息地丧失"、"过度捕捞"这种），
# 给下面 dim_species 表的每个物种关联一个威胁类型，纯参考数据，不接受用户输入
...
# hx：真实物种的主数据表——只放有学名（scientific_name）的正式物种，
# 按学名去重（unique），跟"生境/统称"这种没有具体物种对象的卡片是分开的
...
# hx：海滩 × 物种/生境卡片的关联表（一个海滩下面展示的每一张生物多样性卡片）。
# kind 分三种：species（真实物种，必须关联 dim_species）、habitat（生境介绍）、
# group（统称，比如"候鸟"泛指一类），后两种没有具体物种对象，species_id 为空
# 由下面的 CheckConstraint 强制保证——kind='species' 当且仅当 species_id 不为空。
# occurrence_* 三列是给"物种分布预测"功能用的：occurrence_state 标记这张卡片
# 有没有跑过模型（ready/pending/unavailable），occurrence_score 是模型输出的
# 0-100 相对分数（不是概率），occurrence_basis 说明这个分数是怎么来的——
# 三者必须同步：没有分数就不能是 ready 状态，有分数就必须写清楚依据
...
# 下面一长串 CheckConstraint 是数据库层面的第二道防线——就算应用层校验被绕过
# （比如以后加了新的写入路径），数据库本身也不允许存进不合法的数据，
# 比如"manual 来源却带了坐标"或"六个品类列全是空"这种情况直接会被数据库拒绝
reports_table = Table(
    "reports",
    metadata,
...
# hx：两个索引分别对应两个最高频的查询——按海滩+状态+时间算严重度（beach_summary），
# 按举报人+海滩+时间查重（duplicate_status）；没有这两个索引，数据量大了以后
# 这两个查询会变成全表扫描
...
# hx：启动时依次做几件事——把旧表名迁移过来、建表（含新的 beaches/dim_threat/
# dim_species/area_species 四张表）、按需建一个演示用的固定编号账号、
...
# 所以这里手动检查 reports 表缺哪些列（各品类数量列、坐标、状态备注、
# 更新/软删时间这些），缺了就用 ALTER TABLE 补上，保证老数据库升级后
# 新字段也能正常读写
...
# hx：四个海滩的信息优先从数据库读（seed_reference_data 灌进去的那份），
...
# hx：把四个海滩和它们的生物多样性卡片种子数据灌进数据库，只在启动时补齐，
# 不会覆盖已有数据（用数据库原生的 "ON CONFLICT DO NOTHING" 语法，SQLite 和
# PostgreSQL 各自的方言不一样，所以要分开导入；万一是别的数据库方言，就退化成
# "先查存不存在，不存在才插入"这种笨办法，效果一样但不是原子操作）。
# 物种 id 用 uuid5 按学名确定性生成——同一个学名不管跑多少次都会算出同一个 id，
# 不会每次启动都插入重复的物种记录
def seed_reference_data(engine: Engine, beaches: list[dict[str, Any]]) -> None:
    ...
    # hx：这个内部小工具统一处理"插入，但已存在就跳过"的逻辑，
    # 优先用数据库自带的 upsert 语法（更快、原子性更好），没有对应方言支持时
    # 才退回手动查询再插入
...
# 把海滩+生物多样性种子数据灌进数据库、加载物种分布预测模型、
# 先清理一遍孤儿照片，再给启动时已经存在但还没到期的照片重新挂上定时清理器
# （防止服务重启导致清理任务丢失）
def create_app(
    ...
    # hx：物种分布模型在启动时就加载好一次（.joblib 文件），不是每次请求都重新读盘；
    # 预测过程完全离线，不会实时去查 OBIS 这类外部数据源，也不会把预测结果写回数据库——
    # 纯粹是"输入一对经纬度，返回一个模型算出来的分数"，不留状态
    ...
    # hx：给一对经纬度，返回离线模型预测的物种出现分数——纯计算，不查外部数据源、
    # 不写数据库；坐标不在四个已验证模型覆盖的海域范围内就返回 422
    # OUTSIDE_MODEL_AREA，不是 400（400 是"你传的东西格式不对"，
    # 422 是"格式对，但这个位置模型没法给出结果"）
